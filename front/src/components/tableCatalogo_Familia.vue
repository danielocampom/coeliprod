      ...
      >
        ...
        <template #cell(actions)="row">
          <div class="d-flex justify-content-center">
            <btn-update-catalogo @updatePage="updatePage" :data="{ id: row.item.id, familia: row.item.familia }" />
          </div>
        </template>
...
<script>
import BtnUpdateCatalogo from '@/components/btn_update_catalogo_familia.vue';
import LoginComponent from '@/components/cardLogin.vue';
import { fetchApi, refreshSession } from '@/service/service.js';

export default {
  name: 'CatalogoFamilia',
  components: {
    BtnUpdateCatalogo,
    LoginComponent,
  },
  ...
  data() {
    return {
      ...
      tableKey: 0, // Añadido para forzar re-renderizado de la tabla
    };
  },
  watch: {
    refreshKey() {
  ...
  methods: {
    async fetchData() {
      this.isLoading = true;
      try {
        const response = await fetchApi(`${this.apiUrl}prenda/familia/get`, 'GET', this.$session.get('token'));
        ...
        if (response.status === 200) {
          // Asegurar reactividad asignando un nuevo array
          this.items = [...response.datos];
          this.totalRows = this.items.length;
          this.tableKey += 1; // Forzar re-renderizado de la tabla
        }
      } catch (error) {
    ...
    onFiltered(filteredItems) {
      ...
      this.currentPage = 1;
      this.tableKey += 1; // Forzar re-renderizado al filtrar
    },
    clearFilter() {
      this.filter = '';
      this.tableKey += 1; // Forzar re-renderizado al limpiar el filtro
    },
    onFilter() {
      this.currentPage = 1;
    },
