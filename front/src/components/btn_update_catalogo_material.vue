    ...
    <vs-dialog blur v-model="dataModal">
      ...
      <div class="con-form">
        <vs-input
          ...
          type="text"
          v-model="material"
          label-placeholder="Nombre del Material"
        >
          <template #icon>
...
export default {
  ...
  watch: {
    // Observa cambios en la prop 'data' para actualizar 'material'
    "data.material"(newValue) {
      this.material = newValue || "";
    },
  },
  mounted() {
    // Inicializa 'material' con el valor de la prop
  ...
  methods: {
      ...
      if (status == 200) {
        ...
        try {
          const res = await fetch(
            `${this.url}prenda/material/del/${this.data.id}`,
            {
              method: "DELETE",
              headers: {
    ...
    async update() {
      if (!this.material.trim()) {
        this.openNotification(
          "Error",
          "El nombre de la material no puede estar vacío",
          "danger",
          "top-left",
          `<box-icon name='bug' color="#fff"></box-icon>`
        );
      ...
      try {
        let res = await fetch(`${this.url}prenda/material/edt`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Access-Control-Allow-Origin": "*",
